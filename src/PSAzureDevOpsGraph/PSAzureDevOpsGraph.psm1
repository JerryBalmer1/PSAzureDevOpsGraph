...
# what ships. This file exists so the manifest in src/ is importable directly:
# RootModule is a relative reference resolved beside the manifest, so without a
# psm1 here `Import-Module ./src/PSAzureDevOpsGraph/PSAzureDevOpsGraph.psd1`
# fails outright and everything downstream fails for a reason that looks like
# something else.
#
...
# filenames, manifest and the generated export call now has a fourth corner,
# and the suite does not grade this one - keep it in step by hand.
